...
class EventCalendar(HTMLCalendar):
    ...
    day_full = ["Mandag", "Tirsdag", "Onsdag", "Torsdag", "Fredag", "Lørdag", "Søndag"]
    ...
    def formatmonth(self, theyear, themonth, withyear=True):
        ...
        weeks = [
            self.formatweek(week) for week in self.monthdays2calendar(theyear, themonth)
        ]
        week_string = "\n".join(weeks)
        end_tag = "</div>"
...
def group_events_by_day(events, month):
    ...
    for e in events:
            ...
            e.event_start
            if (e.event_end is None or e.event_end < e.event_start)
            else e.event_end
        )
