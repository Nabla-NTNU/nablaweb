...
def _validate_parameter(parameter, value):
    if type(parameter) is not str and type(parameter) is not unicode:
        raise BPCClientException(
            "'%s': Parameter names must be type 'str' or 'unicode', not '%s'." % (parameter, type(parameter)))
...
def _create_valid_request(data):
    ...
    _copy_parameters(REQUESTS[request['request']]['required'], raise_error=True,
                     error_msg="Parameter '%s' is required for request type '%s'." % ('%s', request['request']))
